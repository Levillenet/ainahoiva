...
import { ArrowLeft, Phone, Pill, Users, Smile, Utensils } from 'lucide-react';
...
const ElderDetail = () => {
  ...
  useEffect(() => {
    if (!id) return;
    const fetch = async () => {
      const [elderRes, medsRes, familyRes, reportsRes] = await Promise.all([
        ...
        supabase.from('call_reports').select('*').eq('elder_id', id).order('called_at', { ascending: false }).limit(20),
      ]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-lg p-6 border border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-custom flex items-center gap-2"><Phone className="w-4 h-4" /> {elder.phone_number} {age && `· ${age} vuotta`}</p>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm text-muted-custom">{elder.is_active ? 'Aktiivinen' : 'Ei aktiivinen'}</span>
      ...
      <div className="bg-card rounded-lg p-6 border border-border">
        ...
        {moodData.length > 0 ? (
          ...
          <p className="text-muted-custom text-center py-8">Ei mielialadataa vielä.</p>
        )}
      </div>
        ...
        <div className="bg-card rounded-lg p-6 border border-border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              ...
              <p className="text-xs text-muted-custom">Mieliala</p>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-custom">Soitettu</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-card rounded-lg p-6 border border-border">
        ...
        {meds.length === 0 ? (
          <p className="text-muted-custom">Ei lääkkeitä lisätty.</p>
        ) : (
      ...
      <div className="bg-card rounded-lg p-6 border border-border">
        ...
        {family.length === 0 ? (
          <p className="text-muted-custom">Ei omaisia lisätty.</p>
        ) : (
          <div className="space-y-3">
            {family.map(f => (
              <div key={f.id} className="bg-muted rounded-lg p-4">
                ...
                <p className="text-muted-custom text-sm">{f.phone_number} {f.email && `· ${f.email}`}</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-card rounded-lg p-6 border border-border">
        ...
        {reports.length === 0 ? (
          <p className="text-muted-custom">Ei raportteja vielä.</p>
        ) : (
          <div className="space-y-3">
            {reports.map(r => (
                ...
                <div className="bg-muted rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-4 text-sm">
                      ...
                      <span className="text-muted-custom">{r.duration_seconds ? `${Math.floor(r.duration_seconds / 60)} min` : ''}</span>
                  ...
                  <CollapsibleContent>
                    <p className="text-muted-custom text-sm mt-2 whitespace-pre-wrap">{r.transcript}</p>
                  </CollapsibleContent>
                </div>
              </Collapsible>
            ))}
